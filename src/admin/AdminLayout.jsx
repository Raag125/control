...
const NAV = [
  ...
  { to: '/admin/visitors',  label: 'Visitors',  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg> },
  { to: '/admin/services',  label: 'Services',  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93l-1.41 1.41M4.93 4.93l1.41 1.41M4.93 19.07l1.41-1.41M19.07 19.07l-1.41-1.41M12 2v2M12 20v2M2 12h2M20 12h2"/></svg> },
  ...
  return (
    <div className="adm">
      ...
      <nav className="adm-bottom-nav">
        <button className="adm-bottom-nav__item adm-bottom-nav__menu-trigger" onClick={() => setSidebarOpen(true)}>
        ...
        {NAV.map(n => {
          const isActive = pathname.startsWith(n.to)
          return (
            <Link key={n.to} href={n.to} className={`adm-bottom-nav__item ${isActive ? 'active' : ''}`}>
              {n.icon}
              <span>{n.label}</span>
            </Link>
          )
