  ...
  return (
    <div>
      ...
      <div className="adm-section-header">
        <div>
          <h1 className="adm-section-title" style={{ fontSize: '1.2rem' }}>💳 Payment Records</h1>
            ...
            <table className="adm-table">
              ...
              <tbody>
                {filtered.length === 0
                  ...
                  : filtered.map(p => (
                    <tr key={p.id}>
                      <td style={{ fontFamily: 'monospace', fontSize: '.72rem', color: 'var(--a-green2)', fontWeight: 700 }}>{p.id}</td>
                      <td style={{ fontFamily: 'monospace', fontSize: '.72rem' }}>{p.orderId}</td>
                      <td style={{ fontWeight: 600, fontSize: '.84rem' }}>{p.customer}</td>
                      ...
                      <td style={{ fontSize: '.72rem', fontFamily: 'monospace', color: 'var(--a-muted)' }}>{p.reference || '—'}</td>
                      <td style={{ fontSize: '.72rem', color: 'var(--a-muted)' }}>{p.date ? new Date(p.date).toLocaleDateString('en-IN') : '—'}</td>
                      <td>
            ...
            {filtered.map(p => (
              <div key={p.id} className="adm-mobile-card">
                <div className="adm-mobile-card__header">
                  <div>
                    <span style={{ fontFamily: 'monospace', fontWeight: 800, color: 'var(--a-green2)', fontSize: '.78rem' }}>{p.id}</span>
                ...
                <div className="adm-mobile-card__grid">
                  ...
                  <div className="adm-mobile-card__row">
                    ...
                    <span className="adm-badge adm-badge--gray" style={{ textTransform: 'uppercase', fontSize: '.62rem' }}>{p.method}</span>
                  </div>
                  <div className="adm-mobile-card__row">
                ...
                {p.reference && (
                  <div style={{ fontSize: '.7rem', fontFamily: 'monospace', color: 'var(--a-muted)', background: 'var(--a-card2)', padding: '.35rem .6rem', borderRadius: '6px', border: '1px solid var(--a-border)' }}>
                    Ref: {p.reference}
                  </div>
                )}
