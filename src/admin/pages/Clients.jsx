  ...
  return (
    <div>
      ...
      <div className="adm-section-header">
        <div>
          <h1 className="adm-section-title">👥 Customer Leads &amp; Inquiries</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: '.6rem', marginTop: '.75rem', marginBottom: '1rem' }}>
        {[
          ...
          <div key={s.label} className="adm-card" style={{ padding: '.75rem .85rem', textAlign: 'center' }}>
            <div style={{ fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.value}</div>
            <div style={{ color: 'var(--a-muted)', marginTop: '.2rem', fontWeight: 600 }}>{s.label}</div>
          </div>
        ))}
      </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '.65rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem' }}>
                      <div className="adm-avatar" style={{ width: '38px', height: '38px', flexShrink: 0 }}>
                        {client.name?.[0]?.toUpperCase() || '?'}
                      </div>
                      <div>
                        <div style={{ fontWeight: 800, color: 'var(--a-text)', lineHeight: 1.2 }}>
                          {client.name || 'Anonymous Lead'}
                        </div>
                        <div style={{ color: 'var(--a-muted)', marginTop: '.15rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '.35rem', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                      ...
                      <span className="adm-badge adm-badge--gray">{srcLabel}</span>
                    </div>
                  </div>
                  ...
                  {/* Message Quote */}
                  {client.message && (
                    <p style={{ color: 'var(--a-muted)', fontStyle: 'italic', lineHeight: 1.4, padding: '.35rem .5rem', background: 'rgba(22,163,74,0.04)', borderRadius: '6px', borderLeft: '3px solid var(--a-green2)' }}>
                      "{client.message}"
                    </p>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: '.4rem', flexWrap: 'wrap', paddingTop: '.5rem', borderTop: '1px solid rgba(22,163,74,0.1)' }}>
                    ...
                    <select
                      ...
                      style={{ flex: 1.2, minHeight: '38px', padding: '.2rem .5rem' }}
